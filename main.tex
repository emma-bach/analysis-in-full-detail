...
\begin{document}
	...
	\chapter{First Steps in Differential Geometry}
		...
		\end{definition}
		Under the convention that the $0$th derivative of $f$ is $f$ itself, a $C^0$-diffeomorphism is the same thing as a homeomorphism. If a source uses the term "diffeomorphism" without specifying the degree $n$, it is generally assumed that $n = 1$.
		\begin{corollary}
			A linear map is a $C^1$-diffeomorphism if and only if it is an isomorphism.
		\end{corollary}
		\begin{proof}
			Let $\phi(x) = Ax$ be a linear map.
			\begin{enumerate}
				\item If $\phi(x)$ is a $C^1$-diffeomorphism, it must be invertible, and thus an isomorphism.
				\item Since $\phi$ is linear, it is continuously differentiable with $D\phi(x) = A$. If $\phi$ is an isomorphism, then the inverse $\phi^{-1}(x)$ is also linear, and thus also continuosly invertible. Thus, $\phi$ is a $C^1$-diffeomorphism.
			\end{enumerate}
		\end{proof}
		...
		\begin{importanttheorem}{}{}
			...
			\begin{enumerate}
				...
				\item $M$ is locally given as a $C^r$-level set, i.e. For every $p \in M$ there exists an open neighbourhood $\Omega \subset \bR^{n+k}$ and an $r$-times continuously differentiable function $f : \Omega \to \bR^k$ such that:
				\begin{enumerate}
					\item $\displaystyle M \cap \Omega = \lr{x \in \Omega : f(x) = \vec0_k}$
					\item For all $x \in \Omega$, we have $\rank Df(x) = k$.
				\end{enumerate}
				\item For every $p \in M$, there exists an open neighbourhood $U \times V \subset \bR^n \times \bR^k$, an $r$-times continuously differentiable map $g : R^n \supset U \to V \subset \bR^k$ and a coordinate permutation $\sigma : \bR^{n+k} \to \bR^{n+k}$ such that
				\begin{align*}
					M \cap \sigma(U \times V) = \sigma\lr{(x, g(x)) : x \in U}
				\end{align*}
			\end{enumerate}
		\end{importanttheorem}
		\begin{proof}
			\phantom{}
			\begin{enumerate}
				\item[] $1 \implies 2$: Assume that $m$ is an $n$-dimensional $C^r$ submanifold of $\bR^{n+k}$. By definition, there exists a $C^r$ diffeomorphism $\phi : \bR^{n+k} \to \bR^{n+k}$ such that for every point $p$, there exists an open neighborhood $\Omega \subset \bR^{n+k}$ such that
				\begin{align*}
					\phi(M \cap \Omega) = \lr{x \in \phi(\Omega) \mid x \in \bR^n \times \lr{\vec0_k}}
				\end{align*}
				Since $\phi$ is a diffeomorphism, $D\phi(x)$ is invertible for all $x \in \Omega$. Let $\pi^\bot : \bR^{n} \times \bR^k \to \bR^k$ be the orthogonal projection onto $\bR^k$. Let $f = \pi^\bot \circ \phi$. Then, for any $x \in \Omega$, we get:
				\begin{align*}
					\lr{x \in \Omega : f(x) = 0}
					&= \lr{x \in \Omega : \phi(x) \in \bR^n \times \lr{\vec 0_k}}\\
					&= \phi^{-1}(\lr{x \in \Omega : x \in \bR^n \times \lr{\vec 0_k}})\\
					&= M \cap \Omega
				\end{align*}
				And by the chain rule, $f$ is $r$-times differentiable and has rank $k$.
				\item[] $2 \implies 3$: Let $f$ be as in $2$. Then we can choose $\sigma$ such that $D_yf(x)$ is invertible, after which $3$ follows from the implicit function theorem. 
				\item[] $3 \implies 1$: Follows with $\phi(x,y) := \sigma(x, y - g(x))$, which is a $C^r$-diffeomorphism by the inverse function theorem.
			\end{enumerate}
		\end{proof}
		\begin{definition}
			Let $M \subset \bR^{n+k}$. We define the \tbf{relative Topology on $M$} by letting a set $V \subset M$ be open in $M$ iff. for every $p \in M$ there exists an $\epsilon > 0$ such that
			\begin{align*}
				M \cap B^{n+k}_\epsilon(p) \subset V
			\end{align*}
		\end{definition}
		...
		\begin{importanttheorem}{Tangent Space}{}
			...
			\newpar
			In particular, given a local description of $M$ as the level set of a function $f : \bR^{n+k} \supset \Omega \to \bR^k$, then for $p \in M \cap \Omega$, we have
			\begin{align*}
				T_pM = \ker Df(p)
			\end{align*}
		\end{importanttheorem}
